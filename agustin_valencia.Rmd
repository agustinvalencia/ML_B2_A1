...
EM <- function(K) {
    ...
    for(it in 1:max_it) {
        ...
        if ( it > 1 && abs(llik[it] - llik[it-1]) < min_change || it == max_it){
...
em3 <- EM(3)
em4 <- EM(4)
llik2 <- fun.zero.omit(em2$llik)
llik3 <- fun.zero.omit(em3$llik)
llik4 <- fun.zero.omit(em4$llik)
...
```{r q2c, echo=FALSE}
ggplot() +
    geom_line(aes(x=1:length(llik2), y=llik2), color="black") +
    geom_line(aes(x=1:length(llik3), y=llik3), color="orange") +
    geom_line(aes(x=1:length(llik4), y=llik4), color="darkturquoise") +
    geom_point(aes(x=1:length(llik2), y=llik2), color="black") +
    geom_point(aes(x=1:length(llik3), y=llik3), color="orange") +
    geom_point(aes(x=1:length(llik4), y=llik4), color="darkturquoise") +
    xlab("Iterations") + ylab("log-likelihood")
```

Black curve corresponds to $K=2$, orange to $K=3$ and turquoise to $K=4$
