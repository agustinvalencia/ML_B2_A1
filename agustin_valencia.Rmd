...
for (i in nums) {
    ...
    error_rates_train <- append(error_rates_train, miss)
    
    predicted <- predict(bb, test, type = "class")
    miss <- get_missed(test$Spam, predicted)
    error_rates_test <- append(error_rates_test, miss)
    depths <- append(depths, i)
}

p <- ggplot()
p <- p + geom_line(aes(x = depths, y = error_rates_train), color = "red")
p <- p + geom_line(aes(x = depths, y = error_rates_test), color = "blue")
